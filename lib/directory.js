...
function Directory (dir) {
    ...
    var localPath = dir.substr(z.root.length),
        ...
        parents   = dirSource.dir.split(npath.sep);

    this.name     = dirSource.name;
    this.parent   = dirSource.dir.split(npath.sep).pop();
    this.path     = '.' + localPath + '/';
    this.fullPath = npath.join(z.root, localPath, '/');
    this.index    = z.filetree.length;
};

Directory.prototype.has = function (arg) {
    var contents = fs.readdirSync(npath.join(z.root + this.path));
...
Directory.add = function (data) {
    ...
    if (!exists) {
        ...
        origDir.name = origDir.path.slice(0,-1).split(npath.sep).splice(2).join('/');
        newDir.name = data.path.slice(0,-1).split(npath.sep).splice(2).join('/');
        z.filetree.push(data);
    }
};
...
z.getDir = Directory.getDir = function(string) {
    ...
    if (z.filetree) {
        switch (method){
            case 'path':
                if (firstChar == '.') { var isSource = string.startsWith('source',2);};
                if (firstChar == '/') { var isSource = string.includes('source');};
                if (!isSource) {
